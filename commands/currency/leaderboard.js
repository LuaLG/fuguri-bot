...
module.exports = class BalanceCommand extends Command {
  ...
  run (msg) {
    const db = new Database(path.join(__dirname, '../../userMoney.sqlite'));
    // let balance = jsonfile.readFileSync(path.join(__dirname, '../../data/balances.json'));
    // let balance = jsonfile.readFileSync(path.join(__dirname, '../../data/orbs.json'));
    ...
    let query = db.prepare(`SELECT * FROM moneyset WHERE serverID="${msg.guild.id}" ORDER BY money DESC LIMIT 5`).all();
